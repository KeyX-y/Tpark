...
// 3.根据依赖树生成浏览器可执行的js文件
// 4.将js文件输出到指定文件夹内 dist/boundl
...
const babelParser = require('@babel/parser') // babel解析器（JavaScript 解析器
const traverse = require('@babel/traverse').default  // 和 babel 解析器配合使用，来遍历及更新每一个子节点
const { transformFromAst } = require('@babel/core'); //获取ast语法树

const config = require('./tpark.js')
